...
parser.add_argument('--query_path1', metavar='DIR', nargs='+', default=[], help='path to train data2')
parser.add_argument('--query_path2', metavar='DIR', nargs='+', default=[], help='path to train data2')
parser.add_argument('--task', type=str, default='mosaic', help='Task type: vertical, cross-batch, or mosaic')
...
def data_loader(paths):
    ...
    for path in paths:
        with h5py.File(path, "r") as f:
            Xs.append(np.array(f['matrix/data']).transpose())
    return np.concatenate(Xs, axis=0) if len(Xs) > 1 else Xs[0]
    
def feature_loader(paths):
    if isinstance(paths, list):
        paths = paths[0]
    with h5py.File(paths, "r") as f:
        X = np.array(f['matrix/features']).astype(str)
    return X
...
if args.task == 'vertical' or args.ref_path1 == (args.query_path1[0] if isinstance(args.query_path1, list) else args.query_path1):
    y_pred = (z_a + z_b) / 2
elif args.task == 'cross-batch':
    y_pred = np.concatenate(((z_a+z_b)/2, (z_ua+z_ub)/2), 0)
else: # mosaic integration (default original logic)
