...
parser.add_argument('--path1', metavar='DIR', nargs='+', default='NULL', help='path to train data1')
parser.add_argument('--path2', metavar='DIR', nargs='+', default='NULL', help='path to train data2')
...
def data_loader(paths):
    ...
    for path in paths:
        with h5py.File(path, "r") as f:
            X = np.array(f['matrix/data'], dtype=np.float32).transpose()
    ...
    merged = ad.concat(adatas, index_unique=None)
    del adatas
    gc.collect()
    return merged
