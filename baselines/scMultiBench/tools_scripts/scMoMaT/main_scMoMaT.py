...
def run_scMoMaT(file_paths):
    ...
    return knn_indices, knn_dists, umap_embedding, zs#, marker_score
...
knn_indices, knn_dists, umap_embedding, zs = run_scMoMaT(file_paths)
end_time = time.time()
...
zs_concat = np.concatenate(zs, axis=0) if isinstance(zs, list) else zs
file = h5py.File(args.save_path+"/embedding.h5", 'w')
file.create_dataset('data', data=zs_concat)
file.close()
