...
if (conda_prefix != "") {
  ...
  current_python <- Sys.which("python")
  if (current_python != "") {
    use_python(current_python, required = TRUE)
  }
}
library(MOFA2)
library(reshape2)
...
# Robustly source util.R
loaded <- FALSE
paths_to_try <- c(
  "util.R",
  "tools_scripts/MOFA2/util.R",
  "baselines/scMultiBench/tools_scripts/MOFA2/util.R"
)
args_raw <- commandArgs(trailingOnly = FALSE)
file_arg <- grep("--file=", args_raw, value = TRUE)
if (length(file_arg) > 0) {
  script_dir <- dirname(sub("--file=", "", file_arg))
  paths_to_try <- c(paths_to_try, file.path(script_dir, "util.R"))
}
for (p in paths_to_try) {
  if (!loaded && file.exists(p)) {
    tryCatch({
      source(p)
      loaded <- TRUE
      cat("DEBUG: Successfully sourced util.R from:", p, "\n")
    }, error = function(e) {
      cat("DEBUG: Failed sourcing from:", p, "-", conditionMessage(e), "\n")
    })
  }
}
if (!loaded) {
  stop("Could not load util.R from any expected paths! getwd() is: ", getwd())
}
