    ...
    if not os.path.exists(gtf_filename):
        ...
        if not found:
            ...
            try:
                ...
                print(f"📥 Downloading: {url} ({total_size / block_size:.1f} MB)")
                with open(gtf_filename, 'wb') as file:
                    for data in response.iter_content(block_size):
                        file.write(data)
                        downloaded += len(data)
                        percent = (downloaded / total_size) * 100
                        print(f"\rProgress: {percent:.1f}% ({downloaded / block_size:.1f}/{total_size / block_size:.1f} MB)", end='')
                print("\n✅ Download complete!")
            except Exception as e:
                print(f"\n❌ Auto-download failed: {e}")
                print("Please download manually and place it in the baselines/scMultiBench directory.")
                raise e

    scglue.data.get_gene_annotation(
        rna, gtf=gtf_filename,
        gtf_by="gene_name"
    )
...
# For paired data: average RNA and ATAC embeddings per cell (same latent space)
result = (rna.obsm["X_glue"] + atac.obsm["X_glue"]) / 2
