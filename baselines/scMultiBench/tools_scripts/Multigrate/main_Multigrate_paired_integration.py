...
# ==============================================================================
# Always mock scvi._compat.Literal: both scvi-tools 0.20.x (raises DeprecationWarning
# as exception) and >= 1.0 (removed _compat) have issues with it.
# ==============================================================================
scvi_compat = types.ModuleType("scvi._compat")
try:
    from typing import Literal
except ImportError:
    from typing_extensions import Literal
scvi_compat.Literal = Literal
sys.modules["scvi._compat"] = scvi_compat

# ==============================================================================
# Additional patches: only needed when scvi-tools >= 1.0
# ==============================================================================
def _get_scvi_version():
    try:
        import scvi
        from packaging.version import Version
        return Version(scvi.__version__)
    except Exception:
        return None

_scvi_ver = _get_scvi_version()
_NEED_PATCHES = _scvi_ver is not None and _scvi_ver >= __import__('packaging.version', fromlist=['Version']).Version("1.0")
...
if _NEED_PATCHES:
    ...
    try:
        ...
        scvi.model._utils.parse_use_gpu_arg = scvi.model._utils.parse_device_args
    except Exception as e:
        print(f"Warning: Mocking parse_use_gpu_arg failed: {e}")

    import scvi.model.base
    scvi_model_base_utils = types.ModuleType("scvi.model.base._utils")
    def mock_initialize_model(*args, **kwargs):
        pass
    scvi_model_base_utils._initialize_model = mock_initialize_model
    sys.modules["scvi.model.base._utils"] = scvi_model_base_utils

    try:
        import scvi.train._callbacks
        if not hasattr(scvi.train._callbacks, "SaveBestState"):
            class DummySaveBestState(object):
                ...
                def __getattr__(self, name):
                    def dummy_method(*args, **kwargs):
                        pass
                    return dummy_method
            scvi.train._callbacks.SaveBestState = DummySaveBestState
    except Exception as e:
        print(f"Warning: Mocking SaveBestState failed: {e}")

    try:
        import lightning.pytorch as pl
        orig_pl_init = pl.Trainer.__init__
        def new_pl_init(self, *args, **kwargs):
            if "use_gpu" in kwargs:
                use_gpu = kwargs.pop("use_gpu")
                if use_gpu is True or (isinstance(use_gpu, str) and use_gpu.lower() != "false"):
                    kwargs["accelerator"] = "gpu"
                    kwargs["devices"] = 1
                elif use_gpu is False:
                    kwargs["accelerator"] = "cpu"
            orig_pl_init(self, *args, **kwargs)
        pl.Trainer.__init__ = new_pl_init
    except ImportError:
        pass

    try:
        import pytorch_lightning as pl2
        orig_pl2_init = pl2.Trainer.__init__
        def new_pl2_init(self, *args, **kwargs):
            if "use_gpu" in kwargs:
                use_gpu = kwargs.pop("use_gpu")
                if use_gpu is True or (isinstance(use_gpu, str) and use_gpu.lower() != "false"):
                    kwargs["accelerator"] = "gpu"
                    ...
                    kwargs["accelerator"] = "cpu"
            orig_pl2_init(self, *args, **kwargs)
        pl2.Trainer.__init__ = new_pl2_init
    except ImportError:
        pass

    try:
        import torch.utils.data
        orig_loader_init = torch.utils.data.DataLoader.__init__
        def new_loader_init(self, *args, **kwargs):
            if "use_gpu" in kwargs:
                kwargs.pop("use_gpu")
            orig_loader_init(self, *args, **kwargs)
        torch.utils.data.DataLoader.__init__ = new_loader_init
    except Exception as e:
        print(f"Warning: Mocking DataLoader failed: {e}")
...
def run_Multigrate(file_paths):
    if (file_paths['rna_path'] is not None) and (file_paths['adt_path'] is not None) and (file_paths['atac_path'] is None):
        ...
        model.train(max_epochs=args.epochs, lr=args.lr, batch_size=args.bs, gradient_clip_val=1.0)
