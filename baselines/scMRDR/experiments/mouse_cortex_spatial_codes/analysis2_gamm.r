...
analyze_gene_gamm <- function(gene, expr_df, meth_df, meta_df) { # Added k as an argument
  ...
  tryCatch({
    ...
    conv_code <- model$mer@optinfo$conv$opt # 0 means success
...
analyze_gene_gam <- function(gene, expr_df, meth_df, meta_df) {
  ...
  tryCatch({
    ...
    model <- gam(expr ~ meth * layer + s(x, y, bs = "gp"),
                 data = df,
    ...
    results <- list(
      gene = gene,
      beta_meth_ref_layer = coef_meth, # 明确这是参考层的 beta
      se_meth_ref_layer = se_meth,
      pval_meth_ref_layer = pval_meth,
      adj_r_sq = adj_r_sq,
...
# head(results_df)
# class(results_df)
# write.csv(results_df, "results/rna_methy_cis_gamm4_gp.csv", row.names = FALSE)
...
head(results_df)
class(results_df)
write.csv(results_df, "results/rna_methy_mCG_cis_gamm4_gp.csv", row.names = FALSE)
...
head(results_df)
class(results_df)
write.csv(results_df, "results/rna_methy_mCH_cis_gamm4_gp.csv", row.names = FALSE)
...
head(results_df)
class(results_df)
write.csv(results_df, "results/rna_methy_mCG_cis_gamm4_subset_gp.csv", row.names = FALSE)
# write.csv(results_df, "results/rna_methy_mCG_cis_gamm4_gp.csv", row.names = FALSE)
...
# head(results_df)
# class(results_df)
# write.csv(results_df, "results/rna_methy_mCH_cis_gamm4_subset_gp.csv", row.names = FALSE)
# # write.csv(results_df, "results/rna_methy_mCH_cis_gamm4_gp.csv", row.names = FALSE)
