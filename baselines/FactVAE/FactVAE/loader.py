...
class dataInstance(Dataset):
    def __init__(self,
        ...
        var_mask = self.RNA_adata.var.highly_variable.values
        if isinstance(self.RNA_adata.raw.X, np.ndarray):
            self.RNA_raw_data = self.RNA_adata.raw.X[:, var_mask]
        else:
            self.RNA_raw_data = self.RNA_adata.raw.X[:, var_mask].toarray()
