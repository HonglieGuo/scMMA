...
class VAE(nn.Module):
    ...
    def get_G2P(self):
        gene_normalized = torch.div(self.decoder_RNA.feature.T, self.decoder_RNA.feature.T.norm(dim=1, keepdim=True) + 1e-8)
        peak_normalized = torch.div(self.decoder_ATAC.feature.T, self.decoder_ATAC.feature.T.norm(dim=1, keepdim=True) + 1e-8)
    ...
    def forward(self, rna_inputs=None, rna_X=None, rna_scale_factor=None, rna_beta=None,rna_sigma=0.001,
        ...
        rna_logvar_clamped = torch.clamp(rna_logvar, max=20)
        rna_kl = torch.exp(rna_logvar_clamped) + rna_mu.pow(2) - rna_logvar - 1  # KL divergence between the embedding representation and the Gaussian distribution
        ...
        atac_logvar_clamped = torch.clamp(atac_logvar, max=20)
        atac_kl = torch.exp(atac_logvar_clamped) + atac_mu.pow(2) - atac_logvar - 1
    ...
    def forward_similar_loss(self, rna_inputs=None, atac_inputs=None):
        ...
        rna_norm = rna_codes / (torch.norm(rna_codes, p=2, dim=-1, keepdim=True) + 1e-8) # normalization
        rna_sim_matrix = torch.matmul(rna_norm, rna_norm.T) # cosine similarity
        atac_norm = atac_codes / (torch.norm(atac_codes, p=2, dim=-1, keepdim=True) + 1e-8)
