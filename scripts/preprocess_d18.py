"""
Preprocess scMultiBench D18 dataset (same format as D15).
...
def main():
    base_dir = Path("datasets/raw_datasets/scMultiBench/D18")
    out_dir  = Path("datasets/h5ad/D18/RNA+ATAC")

    rna_h5  = base_dir / "rna.h5"
    atac_h5 = base_dir / "atac.h5"
    cty_csv = base_dir / "cty.csv"

    for p in (rna_h5, atac_h5, cty_csv):
        if not p.exists():
            print(f"❌ Missing: {p}")
            return
    ...
    rna_out = out_dir / "D18-RNA-counts.h5ad"
    rna.write_h5ad(rna_out)
    print(f"  Saved to {rna_out}")

    # ATAC
    print("\n--- Processing ATAC ---")
    atac = convert_h5_to_adata(atac_h5, cty_df)
    atac_out = out_dir / "D18-ATAC-peaks.h5ad"
    atac.write_h5ad(atac_out)
    print(f"  Saved to {atac_out}")

    print("\n✅ D18 preprocessing completed successfully!")
